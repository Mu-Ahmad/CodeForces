...
int main() {
 ...
 w(t) {
  ...
  temp = min(size - arr[0], arr[1] - size);
  temp = max(temp, 0LL);
  arr[0] += temp;
  arr[1] -= temp;
  ans += 2 * temp;
 
  temp = min(size - arr[0], arr[2] - size);
  temp = max(temp, 0LL);
  arr[0] += temp;
  arr[2] -= temp;
  ans += temp;
 
  temp = min(size - arr[1], arr[0] - size);
  temp = max(temp, 0LL);
  arr[1] += temp;
  arr[0] -= temp;
  ans += temp;
 
  temp = min(size - arr[1], arr[2] - size);
  temp = max(temp, 0LL);
  arr[1] += temp;
  arr[2] -= temp;
  ans += 2 * temp;
 
  temp = min(size - arr[2], arr[0] - size);
  temp = max(temp, 0LL);
  arr[2] += temp;
  arr[0] -= temp;
  ans += 2 * temp;
 
  temp = min(size - arr[2], arr[1] - size);
  temp = max(temp, 0LL);
  arr[2] += temp;
  arr[1] -= temp;
  ans += temp;
