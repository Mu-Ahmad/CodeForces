...
vi solve(vpii& valuesXweights, int capacity, int n){
 // Declaring a 2d DP
 vector <vector <int> > dp(n+1, vector<int>(capacity+1, 0));
 
 for (int y=0; y<n; y++){
  for (int x=0; x<=capacity; x++){
   int totalWeight =  x + valuesXweights[y].se;
   int totalValue =  dp[y][x] + valuesXweights[y].fi;
 
   // Including the Item
   if (totalWeight <= capacity)
    dp[y+1][totalWeight] = totalValue;
 ...
 vi ans;
 for (int y=n, x=capacity; y>0; y--){
  if(dp[y][x] == dp[y-1][x]) continue;
 
  ans.pb(y-1);
  x -= valuesXweights[y-1].se;
 }
 return ans;
}
...
int main() {
 ...
 w(t){
  ...
  frange(n) {
   cin >> temp;
   // deb2(temp, frequency[temp]);
   if (frequency[temp]==k) continue;
