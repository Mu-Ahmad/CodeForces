...
int main() {
    ...
    while(t--)
    {
       ...
       vector<int> q = {arr[arr.size()-1] - 1};
       arr.pop_back();
       int ans = 0;
       bool go = true, additional = true;
       while(go)
       {
        ans++;
        // cout << ans << endl;
        go = false;
        additional = arr.size() == 0;
        ...
        if (additional and q.size()) (*max_element(q.begin(), q.end()))--;
 
        if (arr.size())
        {
            q.push_back(arr[arr.size()-1] - 1);
            arr.pop_back();
            additional = false;
            go = true;
        }
